...
除了单个预览页，也支持**直接给课程页链接**：会自动列出该课所有章节的任务点附件
再全部下载，并按「课程名」建子目录。课程页扫描需要登录态，脚本会直接读本机
Firefox 的 cookies.sqlite 复用登录，不用手动复制 Cookie。
...
def parse_course_url(url: str) -> dict | None:
    ...
    courseid, clazzid = low.get("courseid"), low.get("clazzid")
...
def _get_text(url: str, cookie: str | None, timeout: int,
    ...
    for attempt in range(retries):
        try:
            ...
            if e.code in (401, 403):
                raise RuntimeError(
                    f"HTTP {e.code} —— 登录态失效，请先在 Firefox 里登录学习通并打开一次课程页"
                ) from e
        except Exception as e:  # noqa: BLE001
            last = e
        if attempt < retries - 1:
...
def fetch_course_chapters(cookie: str | None, courseid: str, clazzid: str,
    ...
    if not chapters:
        raise RuntimeError(
            "没解析出章节列表 —— 可能是登录态失效，或该链接不是章节页（应含 courseid/clazzid）"
        )
    return chapters
...
def remote_head_md5(url: str, nbytes: int = HEAD_BYTES, timeout: int = 60) -> str:
    """只取远端前 nbytes 字节算 md5。

    服务端不一定支持 Range；不支持时会返回整包，这里只 read(nbytes)，结果照样正确。
    """
    ...
    with urllib.request.urlopen(req, timeout=timeout) as resp:
        return hashlib.md5(resp.read(nbytes)).hexdigest()
...
def compare_with_remote(cookie: str | None, oid: str, path: str,
    ...
    if not length or not st.get("download"):
        return False, length
    if os.path.getsize(path) != length:
        return False, length
    try:
        same = remote_head_md5(st["download"], HEAD_BYTES, timeout) == local_head_md5(path)
    except Exception:  # noqa: BLE001
        same = False
    return same, length
...
def resolve_course_files(cookie: str | None, files: list[dict], outdir: str,
    ...
    for f in files:
        f["outdir"] = outdir
        f["target"] = predict_name(f.get("name") or "", f["objectid"])
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as ex:
        futs = {
            ex.submit(compare_with_remote, cookie, f["objectid"], f["path"], timeout): f
            for f in pending
        }
...
def main() -> int:
    ...
    # 待下载任务: (objectid, 保存目录, 课程里显示的文件名)
    jobs: list[tuple[str, str, str | None]] = [
        (oid, outdir, None) for oid in extract_objectids(raw)
    ]

    course_urls = extract_course_urls(raw)
    if course_urls:
        if not cookie:
            log("[警告] 没读到 Firefox 的登录 cookie，课程页扫描很可能失败")
        for u in course_urls:
            ...
            try:
                ...
                log(f"  扫描失败：{e}")
                continue
            sub = os.path.join(outdir, sanitize(sc["name"], sc["courseid"]))
            os.makedirs(sub, exist_ok=True)
            log(f"  → 共 {len(sc['files'])} 个附件，保存到 {sub}")
            for f in sc["files"]:
                jobs.append((f["objectid"], sub, f.get("name") or None))
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.jobs)) as ex:
        futs = {
            ...
            for oid, job_dir, name_hint in jobs
        }
        for fut in as_completed(futs):
            r = fut.result()
            if r["ok"]:
