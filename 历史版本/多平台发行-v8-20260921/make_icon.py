#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""生成应用图标（朱砂圆角方块 + 白色下载箭头）。

用法: python make_icon.py [输出路径]   # 默认 app.ico

同时输出：
  app.ico            Windows（build.ps1 / spec 用）
  app_preview.png    预览图
  app.icns           macOS（只有在 macOS 上才生成，spec 会用它）
"""
...
def main():
    ...
    # macOS 的 .icns（spec 在 darwin 上会找它；Linux 的 ELF 用不到图标）
    if sys.platform == "darwin":
        icns = out.replace(".ico", ".icns")
        try:
