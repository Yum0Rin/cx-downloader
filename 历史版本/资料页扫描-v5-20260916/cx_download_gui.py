...
class App(ctk.CTk):
    ...
    def _collect_jobs(self) -> list[tuple[str, str, str, str | None, bool]]:
        ...
        for t in self.tasks:
            ...
            jobs.append((t["kind"], t["payload"], t["outdir"] or base,
                         t["name"] or None, bool(t["exists"])))
    ...
    def _run_scan(self, urls, oids, cookie, base):
        ...
        try:
            ...
            for u in urls:
                ...
                outdir = os.path.join(base, core.sanitize(name, info["courseid"]))
                ...
                try:
                    ...
                    self.q.put(("scan_files", (name, outdir, sc["files"])))
                ...
                try:
                    ...
                    core.resolve_course_files(cookie, dfiles, outdir,
                                              self.timeout, progress=lambda m: say(m))
                    self.q.put(("scan_files", (name, outdir, dfiles)))
                except Exception as e:  # noqa: BLE001
                    say(f"资料扫描失败：{e}", "err")
